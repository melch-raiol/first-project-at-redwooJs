import { Link, routes } from '@redwoodjs/router'
import { useMutation } from '@redwoodjs/web'
import { toast } from '@redwoodjs/web/toast'

import { QUERY } from 'src/components/FileUpload/FileUploadsCell'
import { timeTag, truncate } from 'src/lib/formatters'

const DELETE_FILE_UPLOAD_MUTATION = gql`
  mutation DeleteFileUploadMutation($id: Int!) {
    deleteFileUpload(id: $id) {
      id
    }
  }
`

const FileUploadsList = ({ fileUploads }) => {
  const [deleteFileUpload] = useMutation(DELETE_FILE_UPLOAD_MUTATION, {
    onCompleted: () => {
      toast.success('FileUpload deleted')
    },
    onError: (error) => {
      toast.error(error.message)
    },
    // This refetches the query on the list page. Read more about other ways to
    // update the cache over here:
    // https://www.apollographql.com/docs/react/data/mutations/#making-all-other-cache-updates
    refetchQueries: [{ query: QUERY }],
    awaitRefetchQueries: true,
  })

  const onDeleteClick = (id) => {
    if (confirm('Are you sure you want to delete fileUpload ' + id + '?')) {
      deleteFileUpload({ variables: { id } })
    }
  }

  return (
    <div className="rw-segment rw-table-wrapper-responsive">
      <table className="rw-table">
        <thead>
          <tr>
            <th>Id</th>
            <th>Name</th>
            <th>Size</th>
            <th>Type</th>
            <th>Created at</th>
            <th>&nbsp;</th>
          </tr>
        </thead>
        <tbody>
          {fileUploads.map((fileUpload) => (
            <tr key={fileUpload.id}>
              <td>{truncate(fileUpload.id)}</td>
              <td>{truncate(fileUpload.name)}</td>
              <td>{truncate(fileUpload.size)}</td>
              <td>{truncate(fileUpload.type)}</td>
              <td>{timeTag(fileUpload.createdAt)}</td>
              <td>
                <nav className="rw-table-actions">
                  <Link
                    to={routes.fileUpload({ id: fileUpload.id })}
                    title={'Show fileUpload ' + fileUpload.id + ' detail'}
                    className="rw-button rw-button-small"
                  >
                    Show
                  </Link>
                  <Link
                    to={routes.editFileUpload({ id: fileUpload.id })}
                    title={'Edit fileUpload ' + fileUpload.id}
                    className="rw-button rw-button-small rw-button-blue"
                  >
                    Edit
                  </Link>
                  <button
                    type="button"
                    title={'Delete fileUpload ' + fileUpload.id}
                    className="rw-button rw-button-small rw-button-red"
                    onClick={() => onDeleteClick(fileUpload.id)}
                  >
                    Delete
                  </button>
                </nav>
              </td>
            </tr>
          ))}
        </tbody>
      </table>
    </div>
  )
}

export default FileUploadsList
